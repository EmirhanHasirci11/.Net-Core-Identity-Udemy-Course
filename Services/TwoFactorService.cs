...
namespace IdentityUdemyCourse.Services
{
    public class TwoFactorService
    {
        ...
        public string GenerateQrCodeUrl(string email, string unformatedKey)
        {
            const string format = "otpauth://totp/{0}:{1}?secret={2}&issuer={0}&digits=6";
            return string.Format(format, _urlEncoder.Encode("www.Honoso.com"), _urlEncoder.Encode(email), unformatedKey);
